...
## 5. The Prime Digest (Bridge to External Mind)
To enable external AI models (Ants) to synchronize with the Hologram, the system generates a **Prime Digest**.
- **Artifact**: `TXT/m32/PRIME_DIGEST.txt`.
- **Content**: A unified manifest where every entry consists of the **Physical Address**, the **Semantic Identity**, and the **Provenance Hash**.
- **Role**: This file is the "First Impression" for any new intelligence. It provides the radial map necessary to navigate the prism without distortion.
- **Morphic Seed**: Future expansions (e.g., `mind.sigma`) are generated by agents processing this digest and transmuting intent into new spectral coordinates.
